...
class DataIngestion:
    ...
    def download_data_from_url(self, url: str) -> str:
        try:
    ...
    def extract_tgz_file(self, tar_file_path: str) -> None:
        try:
    ...
    def split_data(self, raw_data_dir: str) -> None:
    ...
    def initiate_data_ingestion(self) -> DataIngestionArtifacts:
        ...
        tgz_file_path = self.download_data_from_url(url)
        self.extract_tgz_file(tgz_file_path)
        raw_data_dir = self.data_ingestion_config.raw_data_path
        train_df, test_df = self.split_data(raw_data_dir)
        train_dir = self.data_ingestion_config.train_data_dir
        test_dir = self.data_ingestion_config.test_data_dir
        os.makedirs(train_dir, exist_ok=True)
        os.makedirs(test_dir, exist_ok=True)
        train_df.to_csv(path_or_buf=f'{train_dir}/train.csv', index=False)
        test_df.to_csv(path_or_buf=f'{test_dir}/test.csv', index=False)
        ...
        return DataIngestionArtifacts(
            train_file_path=f'{train_dir}/train.csv',
            test_file_path=f'{test_dir}/train.csv',
            is_ingested=True,
